...
import { Avatar, Button, Card, CardHeader, Input, Pagination } from "@nextui-org/react";
...
const Search = () => {
    ...
    const fetchSearchedUsers = async (searchTerm: string | null, page: number = 1) => {
        ...
        setIsLoading(true);
        const users: Array<GeneralUser> = (await api.get(`/user/search?searchTerm=${searchTerm}&page=${page}`)).data;
